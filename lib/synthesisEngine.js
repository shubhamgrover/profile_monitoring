...
export function synthesizeCompanyAccount(companyName, data = {}, targetDept = 'Marketing', gtmSettings = null) {
  ...
  if (companyName.toLowerCase().includes('factors.ai') || companyName.toLowerCase().includes('factors')) {
    ...
    if (hasSalesJobs || isHR) {
      ...
      strategicCorrelations.push({
        title: isHR ? "Sales Compensation & Enablement Alignment" : "Multi-Channel Brand Velocity & Audience Capture",
        evidence: "recruitment [sales positions] + company-data [target metrics]",
        narrative: isHR
          ? `Since we see active sales team hiring, it means revenue targets are increasing (implication) and reps need fast productivity (L&D need), and generally you need specific sales training to have everyone up to speed.`
        ...
        friction: isHR
          ? "Aligning sales incentives, commission structures, and quota architectures to prevent new hire attrition."
          : "Capitalizing on organic content attention spikes and converting brand engagement into trusted audience access.",
        script: isHR
          ? `Which means this can be our pitch: "I noticed your active sales hiring. Since sales scaling exists, it generally means new reps need faster productivity. We help HR and rewards leaders leverage Aon's ${courseInfo.name} program to upskill your compensation teams in ${courseInfo.benefit} to get everyone up to speed. Worth a quick chat?"`
    ...
    if (reddit.length > 0 || news.length > 0 || isHR) {
      ...
      strategicCorrelations.push({
        title: isHR ? "Organizational Restructuring & Change Integration" : "Community Backchannel & Media Coverage Amplification",
        evidence: "pr-news [media] + reddit-discussions [discussions]",
        narrative: isHR
          ? `Since we see PR transition announcements or discussions, it means teams are combining or processes are shifting (implication) and people managers need change management guidance (L&D need), and generally you need strategic HR leadership to have everyone up to speed.`
        ...
        friction: isHR
          ? "Equipping people operations managers with frameworks to standardize culture and prevent employee churn."
          : "Building thought leadership, executive trust, and category authority across premium media channels.",
        script: isHR
          ? `Which means this can be our pitch: "I saw the recent media coverage surrounding ${companyName}'s strategic transition. Since organization shifts exist, it generally means people managers need change management skills. We help companies leverage Aon's ${courseInfo.name} to train HR leaders in ${courseInfo.benefit} to get everyone up to speed. Worth a touchpoint?"`
    ...
    if (sitemaps.length > 0 || isHR) {
      ...
      strategicCorrelations.push({
        title: isHR ? "Digital Workflows & Tech Adoption Audits" : "Marketing Tech Stack Integration & Sync Play",
        evidence: "website-intelligence [sitemaps] + tech-stack [enrichment]",
        narrative: isHR
          ? `Since we see system upgrades and web updates, it means technology adoption is underway (implication) and employees need training to reduce implementation failures (L&D need), and generally you need HR technology training to have everyone up to speed.`
        ...
        friction: isHR
          ? "Training HR ops and systems managers on people analytics, tool integrations, and data management."
          : "Eliminating data silos and targeting audiences based on clean first-party data syncs.",
        script: isHR
          ? `Which means this can be our pitch: "I saw the recent system updates at ${companyName}. Since tech rollouts exist, it generally means software only succeeds when people adopt it. We help companies leverage Aon's ${techCourse.name} to train HR ops teams in ${techCourse.benefit} to get everyone up to speed. Worth a brief look?"`
    ...
    // Correlation 5: Talent Retention & Compensation Consolidation
    if (isHR) {
      const rewardsCourse = getAonCourse('rewards');
      strategicCorrelations.push({
        title: "Total Rewards & Talent Retention Optimization",
        evidence: "market-data [growth-stack] + intent-engine [buying-signals]",
        narrative: `Since we see team growth and talent scaling, it means attrition risks are increasing (implication) and managers need rewards strategies to retain top performers (L&D need), and generally you need competitive total rewards frameworks to have everyone up to speed.`,
        friction: "Consolidating fragmented HR training vendors and standardizing pay structures across business units.",
        script: `Which means this can be our pitch: "I noticed your active team scaling. Since rapid expansion exists, retaining high-performance talent is critical. We help HR leaders leverage Aon's ${rewardsCourse.name} to train compensation managers in ${rewardsCourse.benefit} to get everyone up to speed. Worth a quick chat?"`
      });
    }
