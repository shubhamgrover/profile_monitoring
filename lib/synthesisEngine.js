...
export function synthesizeCompanyAccount(companyName, data = {}, targetDept = 'Marketing', gtmSettings = null) {
  ...
  const productName = (resolvedGtm.productName && resolvedGtm.productName.trim() !== '') ? resolvedGtm.productName : 'our platform';
  const productDesc = (resolvedGtm.productDesc && resolvedGtm.productDesc.trim() !== '') ? resolvedGtm.productDesc : 'real-time intent signal detection to auto-generate B2B outreach scripts';
  const competitors = (resolvedGtm.competitors && resolvedGtm.competitors.trim() !== '') ? resolvedGtm.competitors : 'traditional ad networks';
