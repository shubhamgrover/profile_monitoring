...
export function synthesizeCompanyAccount(companyName, data = {}, targetDept = 'Marketing') {
  ...
  if (companyName.toLowerCase().includes('factors.ai') || companyName.toLowerCase().includes('factors')) {
    ...
    if (sitemaps.length > 0 && jobs.length > 0) {
      strategicCorrelations.push({
        ...
        script: `Which means this can be our pitch: "I noticed ${companyName} is adding web pages alongside scale-recruiting. Since new sitemaps and hiring exist, it generally means you're launching fresh features. We help de-anonymize the new traffic surge hitting those assets. Worth a brief chat?"`
      });
    }
    ...
    if (youtube.length > 0 || tweets.length > 0 || posts.length > 0) {
      strategicCorrelations.push({
        ...
        script: `Which means this can be our pitch: "Loved the recent brand content drive. Since active social posts and mentions exist, it generally means you are capturing solid market interest. We help companies route B2B intent from these brand spikes directly to reps. Worth a quick chat?"`
      });
    }
    ...
    if (reddit.length > 0 || news.length > 0) {
      strategicCorrelations.push({
        ...
        script: `Which means this can be our pitch: "I saw the recent media coverage and community discussions. Since PR news and public comments exist, it generally means interest in your space is spiking. We help target these specific segments with warm outbound. Worth a touchpoint?"`
      });
    }
    ...
    strategicCorrelations.push({
      ...
      script: `Which means this can be our pitch: "I saw the team scaling. Since new hiring and active growth exist, it generally means keeping rep ramp time under 10 days is a priority. We help sync account intelligence directly to reps. Worth a quick chat?"`
    });
    ...
    strategicCorrelations.push({
      ...
      script: `Which means this can be our pitch: "I was looking at the recent web sitemap updates. Since sitemap additions and active tracking exist, it generally means your marketing ops team is refining integrations. We automate lead sync and alert reps in real-time. Worth a brief look?"`
    });
    ...
    strategicCorrelations.push({
      ...
      script: `Which means this can be our pitch: "I saw the buying intent signals. Since active expansion and intent triggers exist, it generally means you are consolidating growth tools. We help teams replace expensive paid ads with first-party intent tracking. Worth a touchpoint?"`
    });
