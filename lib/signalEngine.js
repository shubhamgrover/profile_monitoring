...
export function detectSignals(profile, prevSnapshot, newSnapshot) {
  ...
  const allPosts = [
    ...
    ...(newSnapshot.activity || []),
    ...(newSnapshot.posts || [])
  ];

  // Combine recent posts and activity
  allPosts.forEach(post => {
    const postTextRaw = post.title || post.text || '';
    if (!postTextRaw) return;
    const text = postTextRaw.toLowerCase();
    ...
    if (text.includes('claude code') || text.includes('gartner') || text.includes('hype cycle') || text.includes('salesforce') || text.includes('fin')) {
      if (text.includes('gartner') || text.includes('hype cycle') || text.includes('cold call') || text.includes('salesforce') || text.includes('fin')) {
        signals.push({
          ...
          company: newSnapshot.currentCompany || profile.company,
          linkedinUrl: profile.linkedinUrl,
          postUrl: post.link || post.url,
          detectedAt: new Date().toISOString(),
          source: 'LinkedIn Activity · Recent',
          evidence: `Engaged with: "${postTextRaw.slice(0, 70)}..."`,
          dismissed: false,
        });
      }
      ...
      if (text.includes('claude code') || text.includes('contextops') || text.includes('knowledge management')) {
        signals.push({
          ...
          evidence: `Post: "${postTextRaw.slice(0, 70)}..."`,
          dismissed: false,
        });
      }
    }
    ...
    if (text.includes('3:01') || text.includes('culture ritual') || text.includes('meeting at atlys')) {
      signals.push({
        ...
        detectedAt: new Date().toISOString(),
        source: 'LinkedIn Post · Recent',
        evidence: `Culture post: "${postTextRaw.slice(0, 70)}..."`,
        dismissed: false,
      });
    }
    ...
    if (actType.includes('liked') && (text.includes('new chapter') || text.includes('started a new') || text.includes('joining') || text.includes('happy to share'))) {
      signals.push({
        ...
        evidence: `Liked new hire post: "${postTextRaw.slice(0, 70)}..."`,
        dismissed: false,
      });
    }

    // --- GENERIC POST SIGNAL DETECTORS (For all posts) ---
    
    // A. Product Launch / Features release
    if (/launch|introducing|announce|excited to share|shipped|features|new release|unveil/i.test(text) && !/liked/i.test(actType)) {
      signals.push({
        id: `launch-${post.id || Math.random()}-${Date.now()}`,
        type: 'product_launch',
        priority: 'urgent',
        label: 'Product / Feature Launch',
        emoji: '🚀',
        why: `<strong>${profile.name}</strong> announced a product or feature launch: <strong>"${postTextRaw.replace(/\n/g, ' ')}"</strong>. New product momentum indicates high development prioritization.`,
        action: 'Congratulate & Pitch',
        profile: profile.name,
        company: newSnapshot.currentCompany || profile.company,
        linkedinUrl: profile.linkedinUrl,
        postUrl: post.link || post.url,
        ...
        evidence: `Launch announcement: "${postTextRaw.slice(0, 70)}..."`,
        dismissed: false,
      });
    }

    // B. Strategic Partnership / Customer Win
    if (/partnership|partnered|collab|collaborate|customer|client|proud to support|selected by/i.test(text)) {
      signals.push({
        id: `partnership-${post.id || Math.random()}-${Date.now()}`,
        type: 'partnership_signal',
        priority: 'week',
        label: 'Partnership or Customer Win',
        emoji: '🤝',
        why: `<strong>${profile.name}</strong> shared a strategic partnership or new customer milestone: <strong>"${postTextRaw.replace(/\n/g, ' ')}"</strong>. Commercial expansion indicates open budget doors.`,
        action: 'Reach out to expand business',
        profile: profile.name,
        company: newSnapshot.currentCompany || profile.company,
        linkedinUrl: profile.linkedinUrl,
        postUrl: post.link || post.url,
        ...
        evidence: `Partnership: "${postTextRaw.slice(0, 70)}..."`,
        dismissed: false,
      });
    }

    // C. Podcast / Interview / Content thought leadership
    if (/podcast|episode|listen|youtube|video|interview|recorded|live stream|speaking at/i.test(text)) {
      signals.push({
        id: `media-${post.id || Math.random()}-${Date.now()}`,
        type: 'thought_leadership',
        priority: 'week',
        label: 'Media or Podcast Shared',
        emoji: '🎙️',
        why: `<strong>${profile.name}</strong> shared a new media piece, podcast, or interview: <strong>"${postTextRaw.replace(/\n/g, ' ')}"</strong>. Great conversational hooks.`,
        action: 'Review content & engage',
        profile: profile.name,
        company: newSnapshot.currentCompany || profile.company,
        linkedinUrl: profile.linkedinUrl,
        postUrl: post.link || post.url,
        ...
        evidence: `Media post: "${postTextRaw.slice(0, 70)}..."`,
        dismissed: false,
      });
    }

    // D. Corporate Funding / Expansion
    if (/funding|raise|round|series|invest|expansion|grew|growth/i.test(text) && !/hiring/i.test(text)) {
      signals.push({
        id: `growth-${post.id || Math.random()}-${Date.now()}`,
        type: 'growth_signal',
        priority: 'urgent',
        label: 'Funding or Growth Event',
        emoji: '📈',
        why: `<strong>${profile.name}</strong> posted about capital growth or corporate expansion: <strong>"${postTextRaw.replace(/\n/g, ' ')}"</strong>. Growth indicates newly opened budgets.`,
        action: 'Pitch scale solution',
        profile: profile.name,
        company: newSnapshot.currentCompany || profile.company,
        linkedinUrl: profile.linkedinUrl,
        postUrl: post.link || post.url,
        detectedAt: new Date().toISOString(),
        source: 'LinkedIn Update',
        evidence: `Growth signal: "${postTextRaw.slice(0, 70)}..."`,
        dismissed: false,
      });
    }
  });
  ...
  // 7. Company posts checks (Extended Generic checking for company posts too!)
  if (newSnapshot.posts && newSnapshot.posts.length > 0) {
    newSnapshot.posts.forEach(cpost => {
      const cpostText = cpost.text || '';
      if (!cpostText) return;
      const text = cpostText.toLowerCase();

      // Atlys Event specific match
      if (text.includes('people leaders') || text.includes('comedy') || text.includes('talent deck')) {
        signals.push({
          ...
          evidence: `Post text: "${cpostText.slice(0, 70)}..."`,
          dismissed: false,
        });
      }

      // Generic Company Page Launch
      if (/launch|introducing|introducing our|announce|we are excited to|release|unveil/i.test(text)) {
        signals.push({
          id: `company-launch-${cpost.id || Math.random()}-${Date.now()}`,
          type: 'product_launch',
          priority: 'urgent',
          label: 'Company Launch / Announcement',
          emoji: '🚀',
          why: `<strong>${newSnapshot.name || profile.company}</strong> announced a launch: <strong>"${cpostText.replace(/\n/g, ' ')}"</strong>. Brand launch updates indicate high product prioritizations.`,
          action: 'Pitch integrations/solutions',
          profile: profile.name,
          company: newSnapshot.name || profile.company,
          linkedinUrl: profile.linkedinUrl,
          detectedAt: new Date().toISOString(),
          source: 'Company Page Post',
          evidence: `Launch: "${cpostText.slice(0, 70)}..."`,
          dismissed: false,
        });
      }

      // Generic Company Page Partnership
      if (/partnership|partnered|collab|collaborate|proud to support|selected by/i.test(text)) {
        signals.push({
          id: `company-partnership-${cpost.id || Math.random()}-${Date.now()}`,
          type: 'partnership_signal',
          priority: 'week',
          label: 'Corporate Partnership / Customer Win',
          emoji: '🤝',
          why: `<strong>${newSnapshot.name || profile.company}</strong> corporate page announced a partnership or new client: <strong>"${cpostText.replace(/\n/g, ' ')}"</strong>. Commercial milestones are strong warm intro triggers.`,
          action: 'Pitch expanding accounts',
          profile: profile.name,
          company: newSnapshot.name || profile.company,
          linkedinUrl: profile.linkedinUrl,
          detectedAt: new Date().toISOString(),
          source: 'Company Page Post',
          evidence: `Partnership: "${cpostText.slice(0, 70)}..."`,
          dismissed: false,
        });
      }
    });
...
/**
 * Dynamically reweights a signal's priority based on target department.
 * - Hiring/recruitment signals targeting matching departments are upgraded to 'urgent'.
 * - Hiring/recruitment signals targeting non-matching departments are downgraded to 'watch'.
 * - Product launch triggers are upgraded to 'urgent' if the target department is 'Product'.
 */
export function reweightSignal(signal, targetDept) {
  if (!targetDept) return signal;
  
  const deptKeywords = {
    'Marketing': ['marketing', 'brand', 'growth', 'pr', 'communications', 'cmo', 'copywriter', 'seo', 'creative'],
    'Sales': ['sales', 'outbound', 'bd', 'business development', 'account', 'revenue', 'cro', 'sdr', 'bdr'],
    'HR': ['hr', 'talent', 'recruiting', 'people', 'culture', 'chro', 'recruiter'],
    'Engineering': ['engineering', 'developer', 'technical', 'software', 'architect', 'cto', 'frontend', 'backend', 'qa', 'coder', 'developer'],
    'Operations': ['operations', 'ops', 'coo', 'admin', 'coordinator'],
    'Product': ['product', 'pm', 'cpo', 'designer', 'ux', 'ui']
  };

  const currentKeywords = deptKeywords[targetDept] || [];
  const labelLower = (signal.label || '').toLowerCase();
  const whyLower = (signal.why || '').toLowerCase();

  // If it's a hiring or talent surge signal
  const isHiring = signal.type === 'hiring_surge' || labelLower.includes('hiring') || labelLower.includes('recruit') || whyLower.includes('hiring') || whyLower.includes('recruit');

  if (isHiring) {
    // Check if it matches target department keywords
    const matchesTarget = currentKeywords.some(kw => labelLower.includes(kw) || whyLower.includes(kw));
    if (matchesTarget) {
      return { ...signal, priority: 'urgent' }; // Reroute to Urgent!
    } else {
      return { ...signal, priority: 'watch' }; // Downgrade different departments to Watch
    }
  }

  // If it's a product launch signal and target is Product
  if (targetDept === 'Product' && (signal.type?.includes('product') || labelLower.includes('product') || labelLower.includes('launch'))) {
